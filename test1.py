...
def solve():
    ...
    pq = []
    # Priority: (negative collected count, dupe count, number of actions)
    ...
    # We don't include resources in visited key, assuming that if we reach the same state
    # (pos + collected + limits), it's redundant.
    # However, if one path used fewer resources, it is strictly better.
    # The PQ orders by steps/collected, not resources.
    # In strictly-limited resource searches, this simple visited set might be imperfect
    # (blocking a path that uses 'safe' resources if a 'risky' path arrived first),
    # but it usually suffices given 11-draws are step-efficient.
